      ...
      <div className="text-gray-500">
        {sidebarItems.map((item, index) => {
          return (
            <NavLink
              to={item.to}
              style={{ "--delay": `${index * 700}ms` }}
              key={item.to}
              className={({ isActive }) =>
                `${isActive ? "bg-rose-300/50 text-black" : "hover:bg-teal-300/30"} flex items-center justify-between gap-4 pl-6 py-1 rounded-r-full cursor-pointer my-2 transition-all duration-1000 ease-in-out ${
                  showSidebar && `animate-slideIn opacity-0 [animation-delay:var(--delay)]`
                }`
              }
            >
